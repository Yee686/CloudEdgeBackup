...
{
	rprintf(FWARNING, "[yee-%s] do_cmd cmd = %s, machine = %s, user = %s\n", who_am_i(), cmd, machine, user);
	for(int i = 0; i < remote_argc; i++)
	{
		rprintf(FWARNING, "[yee-%s] do_cmd remote_argv[%d] = %s\n", who_am_i(), i, remote_argv[i]);
	}
	int i, argc = 0;
...
static void do_server_sender(int f_in, int f_out, int argc, char *argv[])
{
	...
	// 查看是备份还是恢复，显示从命令行接收的所有参数
	if (source_is_remote_or_local == -1)
		rprintf(FWARNING, "[yee-%s] main.c: do_server_sende source_is_remote_or_local has not been set \n", who_am_i());
	else if(source_is_remote_or_local == 0)
	...
	{
		rprintf(FWARNING, "[yee-%s] do_server_sender argv[%d] = %s\n", who_am_i(), i, argv[i]);
	}
	rprintf(FWARNING, "[yee-%s] do_server_sender recovery version = %s\n", who_am_i(), recovery_version);

	// 如果是恢复任务
	if(source_is_remote_or_local == 1 && recovery_version == NULL)
	{
	...
	rprintf(FWARNING, "[yee-%s] do_server_sender backup_version = %s, recovery_version = %s\n", 
			who_am_i(), backup_version, recovery_version);
...
{
	// rprintf(FWARNING, "[yee-%s] do_recv local_name = %s\n", who_am_i(), local_name);
...
static void do_server_recv(int f_in, int f_out, int argc, char *argv[])
{
	...
	if(source_is_remote_or_local == 0 && backup_version == NULL)
	{
		...
		// }
		// rprintf(FWARNING, "[yee-%s] do_server_recv backup version = %s\n", who_am_i(), backup_version);
	}
...
{
	rprintf(FWARNING, "[yee-%s] main.c:child_main start_server\n", who_am_i());
	for(int i = 0; i < argc; i++)
	{
		rprintf(FWARNING, "[yee-%s] main.c:child_main argv[%d] = %s\n", who_am_i(), i, argv[i]);
	}
...
void start_server(int f_in, int f_out, int argc, char *argv[])
{
	...
	rprintf(FWARNING, "[yee-%s] main.c:start_server \n", who_am_i());
	for(int i = 0; i < argc; i++)
	{
		rprintf(FWARNING, "[yee-%s] main.c:start_server argv[%d] = %s\n", who_am_i(), i, argv[i]);
	}
	...
	if (am_sender) {
		...
		source_is_remote_or_local = 1;
		rprintf(FWARNING, "[yee-%s] main.c:start_server this is a *recovery* task(in set)\n", who_am_i());
		
		recv_filter_list(f_in);
		do_server_sender(f_in, f_out, argc, argv);
	} else {
		source_is_remote_or_local = 0;
		rprintf(FWARNING, "[yee-%s] main.c:start_server this is a *backup* task(in set)\n", who_am_i());
		do_server_recv(f_in, f_out, argc, argv);
	}
	
	// rprintf(FWARNING, "[yee-%s] main.c:start_server recovery version = %s\n", who_am_i(), recovery_version);
	// rprintf(FWARNING, "[yee-%s] main.c:start_server backup version = %s\n", who_am_i(), backup_version);
	...
	// 		rprintf(FWARNING, "[yee-%s] do_server_sender argv[%d] = %s\n", who_am_i(), i, argv[i]);
	// 	}
	// 	rprintf(FWARNING, "[yee-%s] do_server_sender backup version = %s\n", who_am_i(), backup_version);
	// }
...
int client_run(int f_in, int f_out, pid_t pid, int argc, char *argv[])
{
	...
	// rprintf(FWARNING, "[yee-%s] main.c: client_run \n", who_am_i());
	if (am_sender) {
		keep_dirlinks = 0; /* Must be disabled on the sender. */
		...
		// rprintf(FWARNING, "[yee-%s] main client run, whole_file = %d\n", who_am_i(), whole_file);

		if(whole_file == 1){
			flist = send_file_list_and_file(f_in, f_out, argc, argv); // 对应full2sync
...
*/
		...
		/*	finish = clock();
			...
			flist = send_file_list(f_out, argc, argv);	// 对应deltasync
...
static int start_client(int argc, char *argv[])
{
	...
	// for(int i = 0; i < argc; i++){
	// 	rprintf(FWARNING, "[yee-%s] main.c: start_client argv[%d] = %s\n", who_am_i(), i, argv[i]);
	// }
	// rprintf(FWARNING, "[yee-%s] main.c: read_batch = %d\n", who_am_i(), read_batch);
	/*finish = clock();
	...
	*/
	...
	if (!read_batch) { /* for read_batch, NO source is specified */
		char *path = check_for_hostspec(argv[0], &shell_machine, &rsync_port);	// 检查第一个地址
		/**
		 * [yee] main.c: start_client argv[0] = /usr/local/bin/pdtsrc/test_file12
		 * [yee] main.c: start_client argv[1] = rsync_backup@k3smaster::backup/
		*/
		if (path) { /* source is remote */
			source_is_remote_or_local = 1;  // source is remote 参数1时远程资源 代表是一个还原任务
			char *dummy_host;
			int dummy_port = 0;
			*argv = path;
			remote_argv = argv;
			...
			source_is_remote_or_local = 0; // source is local 代表是一个备份任务
			am_sender = 1;
	...
	rprintf(FWARNING, "[yee-%s] main.c: this is a *%s* task\n", who_am_i(), source_is_remote_or_local?"recovery":"backup");
...
# endif
	...
	setlocale(LC_CTYPE, "");
#endif
	rprintf(FWARNING,"[yee-%s] argc = %d\n", who_am_i(), argc);
	for (int i = 0; i < argc; i++)
	{
		rprintf(FWARNING, "[yee-%s] main.c pre parse_arguments argv[%d] = %s\n", who_am_i(), i, argv[i]);
	}
	...
	for (int i = 0; i < argc; i++)
	{
		rprintf(FWARNING, "[yee-%s] main.c post parse_arguments argv[%d] = %s\n", who_am_i(), i, argv[i]);
	}

	rprintf(FWARNING, "[yee-%s] main.c: recovery version = %s\n", who_am_i(), recovery_version);
	...
	if (am_server && protect_args) {
		...
		// for (int i = 0; i < argc; i++)
		// {
		// 	rprintf(FWARNING, "[yee-%s] main.c: pre server parse argv[%d] = %s\n", who_am_i(), i, argv[i]);
		// }
		...
		// for (int i = 0; i < argc; i++)
		// {
		// 	rprintf(FWARNING, "[yee-%s] main.c: post server parse argv[%d] = %s\n", who_am_i(), i, argv[i]);
		// }

		// rprintf(FWARNING, "[yee-%s] main.c: post server parse args recovery version  = %s\n", who_am_i(), recovery_version);
	}
	...
	if (am_server) {
		...
		// rprintf(FWARNING, "[yee-%s] main.c: main start_server\n", who_am_i());
		start_server(STDIN_FILENO, STDOUT_FILENO, argc, argv);
	}
