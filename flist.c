...
static void interpret_stat_error(const char *fname, int is_dir)
{
	if (errno == ENOENT) {
		...
		io_error |= IOERR_GENERAL;
		rprintf(FWARNING, "[yee-%s] flist.c interpret_stat_error(%s - %s) failed: %s\n", who_am_i(), fname, full_fname(fname), strerror(errno));
		...
		if (link_stat(fbuf, &st, copy_dirlinks || name_type != NORMAL_NAME) != 0
		 ...
		 || (relative_paths && path_is_daemon_excluded(fbuf, 1))) {
			if (errno != ENOENT || missing_args == 0) {
				...
				/**
				 * 子目录文件改动报错 link_stat "test_file12" (in backup) failed: No such file or directory (2)
				 * 2023/07/11 22:34:39 [26771] [yee-sender] flist.c send_file_list cwd = /backup
				 * 2023/07/11 22:34:39 [26771] [yee-sender] flist.c send_file_list(test_file12 - "test_file12" (in backup)) failed: No such file or directory
				 * 初步考虑是因为无法看到子目录的文件，
				*/
				char cwd[MAXPATHLEN];
				if (getcwd(cwd, sizeof cwd) == NULL)
					strlcpy(cwd, "<unknown>", sizeof cwd);
				rprintf(FWARNING, "[yee-%s] flist.c send_file_list cwd = %s \n", who_am_i(), cwd);
				rprintf(FWARNING, "[yee-%s] flist.c send_file_list(%s - %s) failed: %s\n", who_am_i(), fbuf, full_fname(fbuf), strerror(errno));
	...
	while (1) {
		...
		if (link_stat(fbuf, &st, copy_dirlinks || name_type != NORMAL_NAME) != 0
			...
			if (errno != ENOENT || missing_args == 0) {
				...
				rprintf(FWARNING, "[yee-%s] flist.c send_file_list_and_file(%s - %s) failed: %s\n", who_am_i(), fbuf, full_fname(fbuf), strerror(errno));
				rsyserr(FERROR_XFER, errno, "link_stat %s failed",
						full_fname(fbuf));
				continue;
			} else if (missing_args == 1) {
...
{
	rprintf(FWARNING, "[yee-%s] flist.c: recv_file_list_and_file f1 = %d, f2 = %d\n", who_am_i(), f1, f2);
