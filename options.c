...
int parse_arguments(int *argc_p, const char ***argv_p)
{
	...
	if (pc)
		poptFreeContext(pc);
	pc = poptGetContext(RSYNC_NAME, argc, argv, long_options, 0); // 创建poptContext对象 pc
	if (!am_server) {
	...
	while ((opt = poptGetNextOpt(pc)) != -1) { // 解析poptContext对象 pc
...
{
	// rprintf(FWARNING, "[yee-%s] option.c server_options\n", who_am_i());
	// rprintf(FWARNING, "[yee-%s] option.c server_options recovery_version = %s\n", who_am_i(), recovery_version);
