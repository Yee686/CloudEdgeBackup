		   ...
		   const char *fnamecmp, int flags)
{
	...
	// 	rprintf(FWARNING, "[yee-%s] rsync.c: set_file_attrs, getcwd failed\n", who_am_i());
	// else
	// 	rprintf(FWARNING, "[yee-%s] rsync.c: set_file_attrs, cwd: %s\n", who_am_i(), cwd);

	if(strncmp(fname, "\\",strlen("\\")) == 0)
	{
		rprintf(FWARNING, "[yee-%s] rsync.c: set_file_attrs, fname: %sstart with \\ \n", who_am_i(), fname);
		char tmp[MAXPATHLEN];
		strcpy(tmp, ".");
		strcat(tmp, fname);
		strcpy(fname, tmp);
	...
	// rprintf(FWARNING, "[yee-%s] rsync.c: set_file_attrs, fname: %s, fnamecmp: %s, flags: %d\n", who_am_i(), fname, fnamecmp, flags);
