			...
			const char *fname, int fd, OFF_T total_size)
{
	...
	if (fd_r >= 0 && size_r > 0) {
		// receive_data() -> map_file() map_strcut的初始化
	...
	while ((i = recv_token(f_in, &data)) != 0) {
		...
		if (i > 0) {	/* literal data 不匹配的字面量数据*/
			...
			if ( !task_type_backup_or_recovery_receiver && first_backup == 0 && delta_fp != NULL && data != NULL) {	
		...
		if (mapbuf) {
			map = map_ptr(mapbuf,offset2,len); // 获取匹配的数据块 -> map
		...
		// 对于backup任务 记录增量信息 -- 匹配的块号
		if ( !task_type_backup_or_recovery_receiver && first_backup == 0 && delta_fp != NULL && map != NULL ) {  
			int write_len = -1;
	...
	/*读取结束*/
	if (!task_type_backup_or_recovery_receiver && delta_fp != NULL) {
		fclose(delta_fp);
		delta_fp = NULL;
	}
...
{	
	// rprintf(FWARNING,"[yee-%s] receiver.c: recv_files f_in = %d, f_out = %d local_name = %s \n", who_am_i(), f_in, f_out, local_name);
	// rprintf(FWARNING,"[yee-%s] receiver.c: recv_files this is a *%s* task \n", who_am_i(), source_is_remote_or_local?"recovery":"backup");


	if (source_is_remote_or_local == -1)
	{
		rprintf(FWARNING, "[yee-%s] receiver.c: recv_files source_is_remote_or_local has not been set \n", who_am_i());
	}
	else
	{	
		char cwd[MAXPATHLEN];
		if(getcwd(cwd, sizeof(cwd)) != NULL)
			rprintf(FWARNING,"[yee-%s] receiver.c: recv_files current working directory is: %s \n", who_am_i(), cwd);
		else
			rprintf(FWARNING,"[yee-%s] receiver.c: recv_files getcwd() error \n", who_am_i());

		task_type_backup_or_recovery_receiver = source_is_remote_or_local; 

		if(task_type_backup_or_recovery_receiver == 0)
			rprintf(FWARNING,"[yee-%s] receiver.c: recv_files this is a *backup* task, backup_version = %s \n", who_am_i(), backup_version);
		else
	...
	while (1) {
		...
		ndx = read_ndx_and_attrs(f_in, f_out, &iflags, &fnamecmp_type,
					 xname, &xlen);
		// rprintf(FWARNING, "[yee-%s] receiver.c: recv_files read_ndx_and_attrs ndx = %d, iflags = %d, fnamecmp_type = %d, xname = %s, xlen = %d \n", who_am_i(), ndx, iflags, fnamecmp_type, xname, xlen);
		if (ndx == NDX_DONE) {
		...
		/* recv file data 接收sender发送的匹配数据*/
