...
def get_process_traffic(pid):
    ...
    with open('./process_traffic.txt', 'w') as file:
        while True:
            ...
            try:
                ...
                interval_bytes_sent = sentBytes - recvBytes_pre
                interval_bytes_received = recvBytes - sendBytes_pre
            ...
            output = "[ {:.1f} - {:.1f} sec] sent {:.2f} MB, up_bandwidth {:.2f} MB/s, recv {:.2f} MB, down_bandwidth {:.2f} MB/s".format(
...
def plot_traffic():
    with open('./process_traffic.txt', 'r') as file:
        ...
        plt.legend()
        plt.savefig("process_traffic.png")

if __name__ == '__main__':
    command = "iperf -c localhost -i 1 -t"
