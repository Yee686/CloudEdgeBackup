...
def get_process_traffic(pid, date):
    ...
    with open(f'/sync_worker/traffic/log/fibre/process_traffic_{date}.txt', 'w') as file:
        while True:
...
def plot_traffic(date):
    with open(f'/sync_worker/traffic/log/fibre/process_traffic_{date}.txt', 'r') as file:
        ...
        plt.legend()
        plt.savefig(f"/sync_worker/traffic/plot/fibre/process_traffic_{date}.png")

if __name__ == '__main__':
    command = "python3 fibre_backup.py"
